...
void Tuya::setup() {
  this->set_interval("heartbeat", 15000, [this] { this->send_empty_command_((uint8_t)TuyaCommandType::HEARTBEAT); });
}
...
void Tuya::dump_config() {
  ...
  ESP_LOGCONFIG(TAG, "  Low Power Protocol: %d", this->use_low_power_protocol_);
...
bool Tuya::validate_message_() {
  ...
  if (this->use_low_power_protocol_) {
    this->handle_low_power_command_(command, version, message_data, length);
  } else {
...
void Tuya::handle_command_(uint8_t command, uint8_t version, const uint8_t *buffer, size_t len) {
  ...
  if (this->expected_response_.has_value() && this->expected_response_ == (uint8_t)command_type) {
  ...
  switch (command_type) {
    case TuyaCommandType::HEARTBEAT:
      ...
      if (this->init_state_ == TuyaInitState::INIT_HEARTBEAT) {
        ...
        this->send_empty_command_((uint8_t)TuyaCommandType::PRODUCT_QUERY);
      }
      break;
    case TuyaCommandType::PRODUCT_QUERY: {
      ...
      if (this->init_state_ == TuyaInitState::INIT_PRODUCT) {
        ...
        this->send_empty_command_((uint8_t)TuyaCommandType::CONF_QUERY);
      }
      break;
    }
    case TuyaCommandType::CONF_QUERY: {
      ...
      if (this->init_state_ == TuyaInitState::INIT_CONF) {
        ...
        if (this->gpio_status_ != -1) {
          ...
          this->send_empty_command_((uint8_t)TuyaCommandType::DATAPOINT_QUERY);
        } else {
    ...
    case TuyaCommandType::WIFI_TEST:
      this->send_command_(TuyaCommand{.cmd = (uint8_t)TuyaCommandType::WIFI_TEST, .payload = std::vector<uint8_t>{0x00, 0x00}});
      break;
...
void Tuya::handle_low_power_command_(uint8_t command, uint8_t version, const uint8_t *buffer, size_t len) {
  ...
  switch (command_type) {
    ...
    case TuyaLowPowerCommandType::WIFI_TEST:
      this->send_command_(TuyaCommand{.cmd = (uint8_t)TuyaLowPowerCommandType::WIFI_TEST, .payload = std::vector<uint8_t>{0x00, 0x00}});
      break;

    case TuyaLowPowerCommandType::DATAPOINT_REPORT_STORED:
      // TODO implement
      ESP_LOGE(TAG, "DATAPOINT_REPORT_STORED is not implemented");
      break;

    case TuyaLowPowerCommandType::OBTAIN_DP_CACHE:
      this->send_command_(TuyaCommand{.cmd = (uint8_t)TuyaLowPowerCommandType::OBTAIN_DP_CACHE, .payload = std::vector<uint8_t>{0x00}});
      break;
    ...
    case TuyaLowPowerCommandType::SIGNAL_STRENGTH_QUERY:
      this->send_command_(TuyaCommand{.cmd = (uint8_t)TuyaLowPowerCommandType::SIGNAL_STRENGTH_QUERY, .payload = std::vector<uint8_t>{0x01, 100}});
      break;
...
void Tuya::send_raw_command_(TuyaCommand command) {
  ...
  switch ((TuyaCommandType)command.cmd) {
    case TuyaCommandType::HEARTBEAT:
      this->expected_response_ = (uint8_t)TuyaCommandType::HEARTBEAT;
      break;
    case TuyaCommandType::PRODUCT_QUERY:
      this->expected_response_ = (uint8_t)TuyaCommandType::PRODUCT_QUERY;
      break;
    case TuyaCommandType::CONF_QUERY:
      this->expected_response_ = (uint8_t)TuyaCommandType::CONF_QUERY;
      break;
    case TuyaCommandType::DATAPOINT_DELIVER:
    case TuyaCommandType::DATAPOINT_QUERY:
      this->expected_response_ = (uint8_t)TuyaCommandType::DATAPOINT_REPORT;
      break;
    default:
      break;
  }
...
void Tuya::send_empty_command_(uint8_t command) {
...
void Tuya::send_wifi_status_() {
  ...
  if (network::is_connected()) {
    ...
    // Protocol version 3 and low-power variant support specifying when connected to "the cloud"
    if (this->protocol_version_ >= 0x03 || this->use_low_power_protocol_) {
  ...
  uint8_t command_type = (this->use_low_power_protocol_ ? (uint8_t)TuyaLowPowerCommandType::WIFI_STATE : (uint8_t)TuyaCommandType::WIFI_STATE);
  this->send_command_(TuyaCommand{.cmd = command_type, .payload = std::vector<uint8_t>{status}});
}
...
void Tuya::send_local_time_() {
  ...
  uint8_t command_type = (this->use_low_power_protocol_ ? (uint8_t)TuyaLowPowerCommandType::LOCAL_TIME_QUERY : (uint8_t)TuyaCommandType::LOCAL_TIME_QUERY);
  this->send_command_(TuyaCommand{.cmd = command_type, .payload = payload});
}
#endif
...
void Tuya::send_datapoint_command_(uint8_t datapoint_id, TuyaDatapointType datapoint_type, std::vector<uint8_t> data) {
  ...
  this->send_command_(TuyaCommand{.cmd = (uint8_t)TuyaCommandType::DATAPOINT_DELIVER, .payload = buffer});
}
